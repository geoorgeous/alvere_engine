...
LuaTildeHost* LuaTilde_Command(LuaTildeHost* host, const char* command, void* param1, void* param2) {
    ...
    if (_stricmp(command, "poll") == 0) {
        if (!host)
            return NULL;
        ((LuaHostWindows*)host)->Poll();
        return host;
    }
    if (_stricmp(command, "isconnected") == 0) {
        if (!host)
            return NULL;
        if (!param1)
            return NULL;
    ...
    if (_stricmp(command, "waitfordebuggerconnection") == 0) {
        if (!host)
            return NULL;
        ((LuaHostWindows*)host)->WaitForDebuggerConnection();
        return host;
    }
    if (_stricmp(command, "registerstate") == 0) {
